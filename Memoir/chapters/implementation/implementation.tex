...
\initial{T}he product realisation phase of the Systems Engineering approach will be presented in the current chapter instead of the previous one since the focus of this phase was put in the software part, for the hardware one (mainly structural mounts) being too dependant on the configuration of the existing UAV.
...
That is, the UAV concept encloses the airframe, propulsion, power source and all the other components as described in \cite{arteta2015}; but most importantly, the controller board with the Ardupilot software, considered as the brain of the UAV, which dictates how the vehicle will behave during operation.
...
He/she has direct control of the UAV when flying in manual mode, plus is responsible of the operation of the GCS when the UAV is in Automatic or Guided modes (see Chapter \ref{chap:ardupilot}).
...
A 2.4 GHz radio wave transmits information on the position of the control sticks as a PWM signal directly to the Ardupilot software, as explained in Section \ref{sec:basics}.
...
he final decision on the UAS architecture after the integration of the OCAS is shown in Figure \ref{fig:uas-ocas}.
...
The steps to a successful trade-off study are \cite{otto1991}:
\begin{enumerate}
...
Finally, the ranking of the solutions is performed by combining (multiplying) the weight factors with the ratings given to the alternatives on each of those criteria, to ultimately sum all of those and obtain a single figure for every alternative considered.
...
The choice of processing unit for the OCAS is not nearly as difficult as the sensor case.
...
\begin{description}
	\item[\scshape Power source] As mentioned in the previous section, any portable USB battery pack with at least 5V, 2A continuous current will sufice to power the computer board and its peripherals.
		The one used for the project is the Amazon Basics 5600 mAh battery, which can potentially last more than two hours of operation with the OCAS.
...
This connection is entirely handled by the kernel, the drivers of the adapter itself and the operating system, and needs no further action from the integrator.
...
In this project, the GPIO pins will be used to both trigger the ultrasonic rangefinders and read the returning signal that encodes the information on the distance from the sensor to the detected obstacle.
...
\begin{enumerate}
	...
	\item Echo is the output signal pin.
		...
		Knowing the speed of propagation of sound (given by $a=\sqrt{\gamma R_g T}$) and the time taken for the wave to travel to the obstacle and back, the distance can be calculated with $d=a \cdot t/2$
\end{enumerate}
...
\input{./figures/sonarconnect.tex}
...
In this case, the Linux OS is Raspbian Wheezy, which is a version of Debian adapted to be run on the ARMv7 chip of the Raspberry Pi.
...
At this point it can be useful to collect all the information generated in Chapters \ref{chap:design} and \ref{chap:implementation} to determine what functions and components have still not been covered and shall be implemented within the custom control script.
...
The complete Python files will be included in the Appendices for completeness.
...
The \texttt{Sonar} class defines three different methods which operate the sonar and calculate the distance to the closest obstacle, compute the velocity from distance measurements and evaluate the potential of a collision to trigger the avoiding manoeuvre, respectively, as can be seen in Appendix \ref{app:sonar}.
...
Nevertheless, the effect that reasonable temperature fluctuations have on the final measured distance is relatively small, as shown in Appendix \ref{app:temperature}, and can be effectively neglected, setting $a(T)=340 m/s$ as the standard and constant value for the speed of sound.

Occasionally, the rangefinders give incorrect measurements (probably due to multipath errors, cross-interference or noise) that can make the algorithms believe that the UAV is moving closer or faster to the detected obstacle than reality, which causes false-positive activation of the avoidance procedures.
...
Since $\frac{dx}{dt}\big\rvert_{t_0}=v(t_0)$:
\begin{equation}
...
Thus, the \texttt{Control} class defines the methods \texttt{take()}, \texttt{checkTake()}, \texttt{give()} and \texttt{checkGive()} which, when called in pairs, perform the actions given as arguments (e.g. send a MAVlink command to the UAV) while simultaneously check for a certain condition to be met (for instance: the message was successfully sent, the vehicle has stopped or reached the commanded position, etc.), both in independent parallel threads to avoid interference with the \texttt{main} function or with themselves (the complete code can be found in Appendix \ref{app:control}).
...
A screenshot of the application itself is shown in Figure \ref{fig:gui}.

\input{./figures/gui.tex}

As it can be seen, the two versions have minimal differences.
Mainly, the Windows version, which was used in the early testing phases, provides the options of launching the SITL simulator as well as the Mission Planner GCS software for in-depth analysis of the incoming data (the lower branch in Figure \ref{fig:mavproxy}).

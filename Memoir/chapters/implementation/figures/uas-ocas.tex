\begin{figure}[htbp]
	...
	\begin{tikzpicture}
		...
		\newcommand{\drawocas}[1]{
			\node[#1,fill=teal,minimum height=1.5cm,minimum width=2.5cm](ocas-block){};
		...
		\draw[thick,decorate,decoration={coil,aspect=0,segment length=0.9cm}] (ocas) -- node[below=2mm,sloped]{\small MAVlink (USB)} (uav);
		\draw[thick,decorate,decoration={expanding waves,angle=6,segment length=2mm}] (ocas) -- node[below=2mm,sloped]{\small SSH (WiFi)} (gcs);
